...
import {
  MonthlyRankBoard,
  MonthlyRankBoardRef,
} from "@/app/timesheet/entry/monthly-rank-board";
...
const TimesheetSummary = forwardRef<MonthlyRankBoardRef, TimesheetSummaryProps>(
  ({ adminId, style }, ref) => {
    ...
    return (
      <div style={{ ...defaultStyle, ...style }}>
        {/* บอร์ดอันดับรายเดือน */}
        <MonthlyRankBoard ref={ref} currentAdminId={adminId} variant="wide" />

        {/* การ์ดโปรเจกต์ที่ใช้เวลามากที่สุด */}
        {topProjectUsage && (
          <UsageCard
            title={t(
              "timesheet_components.most_time_spent_project",
              "โปรเจกต์ที่ใช้เวลามากที่สุด"
            )}
        ...
        {topFeatureUsage && (
          <UsageCard
            title={t(
              "timesheet_components.most_time_spent_feature",
              "ฟีเจอร์ที่ใช้เวลามากที่สุด"
            )}
