...
const ApiLogFilter = ({loading, filters, onSearch, onReset}: ApiLogFilterProps) => {
    ...
    const fetchServices = async () => {
        ...
        try {
            const response = await axios.get('/api/v1/logger/find-service');
            const serviceData = response.data?.data || [];

            // แปลงข้อมูลให้เป็น format ที่ Select ต้องการ
            const serviceOptions: ServiceOption[] = serviceData.map((service: any) => ({
                value: service.service_name || service.name,
    ...
    //** โหลดข้อมูล services เมื่อ component mount */
    useEffect(() => {
        fetchServices();
    }, []);
                ...
                <Col xs={24} sm={12} md={8} lg={6}>
                    <Form.Item label="Service Name">
                        <Select
                            ...
                            notFoundContent={servicesLoading ? 'กำลังโหลด...' : 'ไม่พบข้อมูล'}
                        >
