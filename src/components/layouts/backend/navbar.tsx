...
function AlertItem({ icon, label, href, color, index, onNavigate }: AlertItemProps) {
  const colorMap = {
    amber: {
  ...
  return (
    <motion.button
      ...
      transition={{ duration: 0.28, ease: [0.16, 1, 0.3, 1], delay: index * 0.07 }}
    ...
    >
      ...
      <span className="relative flex-shrink-0 w-2 h-2">
        ...
        <span className={`absolute inset-0 rounded-full ${c.ping} animate-ping opacity-70`} />
      </span>
      ...
      {/* Label */}
      <span className={`text-[11.5px] font-semibold leading-none whitespace-nowrap ${c.text}`}>
        {label}
      </span>
