...
export class ApiLogUtils {
    ...
    static getCalledByFromHeader(request: NextRequest): string {
        ...
        console.log(`🔍 [API Log Utils] Raw x-request-user header: "${xRequestUser}" (type: ${typeof xRequestUser})`);

        // ตรวจสอบว่าไม่มี header หรือเป็น "null" string
        if (!xRequestUser || xRequestUser === 'null' || xRequestUser === 'undefined') {
            console.log(`⚠️ [API Log Utils] Invalid header value, returning "api-unknown"`);
            return xRequestUser ?? "unknown-header";
        }

        // ถ้าเป็น plain text (ตัวเลข, ตัวอักษร, underscore, dash) ให้ใช้เลย
        if (/^[\w-]+$/.test(xRequestUser)) {
            console.log(`✅ [API Log Utils] Using plain text user: "${xRequestUser}"`);
            return xRequestUser;
        }
        ...
        try {
            ...
            if (/^[A-Za-z0-9+/]+={0,2}$/.test(xRequestUser)) {
                ...
                console.log(`✅ [API Log Utils] Decoded Base64 user: "${decoded}"`);
                return decoded;
            }

            // ถ้าไม่ใช่ Base64 ลอง URI decode
            const decoded = decodeURIComponent(xRequestUser);
            console.log(`✅ [API Log Utils] URI decoded user: "${decoded}"`);
            return decoded;
        } catch (e) {
            // ถ้า decode ไม่ได้ ใช้ค่าเดิม
            console.log(`⚠️ [API Log Utils] Failed to decode, using raw: "${xRequestUser}"`);
            return xRequestUser;
        }
    }
    ...
    private static logRequest({
        ...
        const {boldRed, reset} = this.COLORS;

        console.log("\n" + "═".repeat(100));
        console.log(`${boldRed}📡  API Request Log${reset}`);
        console.log(`${boldRed}URL${reset}: ${request.url}`);
        console.log(`${boldRed}Method${reset}: ${request.method}`);
        console.log(`${boldRed}Status${reset}: ${statusColor}${status}${reset}`);
        console.log(`${boldRed}Response Time${reset}: ${duration}ms`);
        console.log(`${boldRed}Headers${reset}: ${this.pretty(Object.fromEntries(request.headers.entries()))}`);
        console.log(`${boldRed}Body${reset}: ${this.pretty(requestBody)}`);
        console.log(`${boldRed}Response${reset}: ${this.pretty(responseBody)}`);
