...
import {ApiLogFilters, ApiLogFormData, ApiLogResponse} from '@/types/api-log.type';

const API_BASE_URL = '/api/v1/logger';

//** ดึงรายการ API Logs */
export const GET_API_LOGS = async (filters: ApiLogFilters): Promise<ApiLogResponse> => {
...
export const POST_CREATE_API_LOG = async (data: ApiLogFormData) => {
    const payload = {
        ...
        requestHeader: data.requestHeader ? JSON.parse(data.requestHeader) : undefined,
...
export const PATCH_ARCHIVE_STATUS = async (id: string, isArchived: boolean) => {
    const response = await axios.patch(`${API_BASE_URL}/${id}/archive`, {isArchived});
    return response.data;
};
