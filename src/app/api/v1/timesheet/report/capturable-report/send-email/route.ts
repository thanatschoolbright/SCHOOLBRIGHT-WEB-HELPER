...
// สร้าง Excel Capturable Report แล้วส่งทางอีเมล
export async function POST(request: NextRequest) {
  ...
  const { start_date, end_date, recipients } = parsed.data;

  try {
    // 1. ดึงข้อมูลรายงาน
    const dataResult = await Service.getProjectStats(start_date, end_date);

    // 2. สร้าง Excel buffer
    ...
    const fileName = `Capitalization_Report_${formattedStart}_to_${formattedEnd}.xlsx`;
    ...
    // 3. สร้าง HTML email
    const subject = `[SchoolBright] Capitalization Report วันที่ ${formattedStart} ถึง ${formattedEnd}`;
...
<body style="font-family:'Segoe UI',Arial,sans-serif;background:#f8fafc;padding:32px 20px;margin:0;">
  <div style="max-width:520px;margin:0 auto;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.08);">
    ...
    <div style="background:linear-gradient(135deg,#1a1a2e,#0f3460);padding:32px 36px;text-align:center;">
      ...
      <div style="color:#94a3b8;font-size:13px;margin-top:6px;">รายงานวิเคราะห์ทรัพย์สิน (Asset Capture Analysis)</div>
    </div>

    <div style="padding:32px 36px;">
      <p style="font-size:15px;color:#0f172a;margin:0 0 16px;">สวัสดี,</p>
      <p style="font-size:14px;color:#475569;line-height:1.7;margin:0 0 20px;">
        ไฟล์ Excel <strong style="color:#1e40af;">Capitalization Report</strong> ประจำช่วงวันที่
        <strong>${formattedStart} ถึง ${formattedEnd}</strong> ได้ถูกแนบมาพร้อมกับอีเมลนี้แล้ว
      </p>

      <div style="background:#f1f5f9;border-radius:8px;padding:16px 20px;margin-bottom:24px;border-left:4px solid #3b82f6;">
        <div style="font-size:13px;color:#475569;margin-bottom:8px;font-weight:600;">ไฟล์ที่แนบมาด้วย</div>
        <ul style="margin:0;padding-left:18px;">
          <li style="font-size:13px;font-weight:600;color:#1e3a5f;padding:4px 0;">${fileName}</li>
        </ul>
      </div>

      <div style="background:#ecfdf5;border-radius:8px;padding:14px 18px;margin-bottom:24px;border-left:4px solid #10b981;">
        <div style="font-size:12px;color:#065f46;font-weight:600;margin-bottom:4px;">ข้อมูลรายงาน</div>
        <div style="font-size:13px;color:#047857;">
          โครงการทั้งหมด: <strong>${dataResult.length} โครงการ</strong>
        </div>
        <div style="font-size:13px;color:#047857;">
          ช่วงเวลา: <strong>${formattedStart} — ${formattedEnd}</strong>
        </div>
      </div>
    ...
    return NextResponse.json(
      successResponse({
        data: { sent, failed, total_files: 1, results },
        message_th: `ส่งอีเมล Capitalization Report สำเร็จ ${sent}/${recipients.length} ที่อยู่`,
      }),
    );
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "unknown";
