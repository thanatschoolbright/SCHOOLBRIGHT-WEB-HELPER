...
const buildEffectivePeriod = (
    ...
    // Local end of yesterday (23:59:59.999 local), then convert back to UTC
    const endOfYesterdayLocal = new Date(
    ...
    const endOfYesterdayLocalToUtc = new Date(endOfYesterdayLocal.getTime() - offsetMs);
    ...
    const effectiveEnd =
        scope === "full"
            ...
            : isRequestMonthCurrent
                ? new Date(
                    Math.min(endLocalToUtc.getTime(), endOfYesterdayLocalToUtc.getTime())
                )
                : endLocalToUtc;
