  ...
  try {
    ...
    if (!markdown) {
      return NextResponse.json(
        errorResponse({
          status: 404,
          message_en: `All Gemini model candidates failed. Last error: ${
            lastErrorDetails?.message || "Unknown error."
          }`,
          message_th: "เรียก Gemini ไม่สำเร็จ เนื่องจากลองทุกรุ่นแล้ว.",
