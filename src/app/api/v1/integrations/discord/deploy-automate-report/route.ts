...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (!automateUrl || !automateUrl.startsWith("http")) {
      console.error(
        "Invalid or missing automateUrl (github_page/url) in payload:",
        payload,
      );
    ...
    const discordPayload: any = {
      content: `${mentionUser} มีรายงานการทดสอบอัตโนมัติพร้อมให้ตรวจสอบแล้ว [INFO]`,
      embeds: [
        {
          title: "[SUCCESS] Automated Test Report is ready",
