...
const ALTERNATIVE_WEBHOOK =
  process.env.WEBHOOK_DISCORD_PULL_REQUEST_SERVER;
const DEFAULT_WEBHOOK =
  process.env.WEBHOOK_DISCORD_RELEASE_PRODUCTION_SERVER;
const GITHUB_TOKEN =
...
> = {
  queued: { emoji: "[QUEUED]", color: 0xf1c40f, label: "รอคิว" },
  in_progress: { emoji: "[DEPLOYING]", color: 0x3498db, label: "กำลัง Deploy" },
  completed: { emoji: "[SUCCESS]", color: 0x2ecc71, label: "เสร็จสิ้น" },
};
...
//** คืนเป็นสตริงหลายบรรทัด เช่น "[SUCCESS] **checkout** completed - 5s"
function buildStepLines(steps: Array<Record<string, any>> | undefined): string {
  ...
  const lines = steps.slice(0, MAX_STEPS).map((step, idx) => {
    const name =
      (step.name ?? step.step ?? step.number)?.toString() ?? `Step ${idx + 1}`;
    const status = (
      step.status ??
      step.conclusion ??
      step.step_status ??
    ...
    const emoji = STEP_STATUS_EMOJI[status] ?? "[ITEM]";
    const duration =
...
async function fetchRunJobsSteps(
  repoFullName: string,
  runId: number | string,
...
export async function POST(request: NextRequest) {
  ...
  const payload: GitHubWebhookPayload = await request
    .json()
    .catch(() => ({}) as GitHubWebhookPayload);
  ...
  if (jobPayload) {
    ...
    itemName =
      runPayload?.name ??
      `#${runPayload?.run_number ?? runPayload?.id ?? "run"}`;
    itemStatus = runPayload?.status ?? itemStatus;
    conclusion = runPayload?.conclusion ?? undefined;
    startedAt =
      runPayload?.created_at ?? runPayload?.run_started_at ?? undefined;
    completedAt =
      runPayload?.updated_at ?? runPayload?.completed_at ?? undefined;
  ...
  if (startedAt)
    descriptionParts.push(
      `เริ่มเมื่อ: <t:${Math.floor(new Date(startedAt).getTime() / 1000)}:R>`,
    );
  if (completedAt)
    descriptionParts.push(
      `เสร็จเมื่อ: <t:${Math.floor(new Date(completedAt).getTime() / 1000)}:R>`,
    );
  ...
  const actorValue =
    ...
    payload?.workflow_run?.head_commit?.author?.name ??
    undefined;
  ...
  try {
    const httpResponse: AxiosResponse = await axios.post(
      DISCORD_WEBHOOK,
      payloadToSend,
      {
    ...
    if (
      discordErrorStatus === 404 &&
      ALTERNATIVE_WEBHOOK &&
      ALTERNATIVE_WEBHOOK !== DISCORD_WEBHOOK
    ) {
      try {
        const fallbackResponse: AxiosResponse = await axios.post(
          ALTERNATIVE_WEBHOOK,
          payloadToSend,
          {
            headers: { "Content-Type": "application/json" },
