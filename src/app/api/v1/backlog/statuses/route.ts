...
export async function GET(req: NextRequest) {
  try {
    ...
    const reason = error?.response?.data || error?.message || "Fetch statuses failed";
    return NextResponse.json(
      errorResponse({ status, message_en: typeof reason === "string" ? reason : "Fetch statuses failed", message_th: "ดึงสถานะไม่สำเร็จ", error }),
      { status }
    );
  }
}
