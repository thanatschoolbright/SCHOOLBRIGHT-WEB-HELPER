...
import {Button, Card, Form, Space, Tag, Typography, Pagination, Input, Select, DatePicker, Row, Col, Empty, Spin, Badge, Avatar, Progress, Collapse} from "antd";
import {
    CopyOutlined, 
    EditOutlined, 
    EyeOutlined, 
...
import {MonthlyRankBoard, MonthlyRankBoardRef} from "../entry/monthly-rank-board";
                        ...
                        <Spin spinning={tableLoading}>
                            {paginatedEntries.length === 0 ? (
                                ...
                                >
                                    <Empty
                                        description={
                                            <Typography.Text style={{ fontSize: 16, color: 'var(--ant-text-color-secondary)' }}>
                                ...
                                <Row gutter={[16, 16]}>
                                    {paginatedEntries.map((entry) => {
                                        const statusOption = STATUS_OPTIONS.find(opt => opt.value === entry.status);
                                        const statusLabel = statusOption
                                            ? (i18n.language === "th" ? statusOption.label_th : statusOption.label_en)
                                            : entry.status;
                                        ...
                                        return (
                                            <Col key={entry.id} xs={24} sm={12} lg={8} xl={6}>
                                                ...
                                                >
                                                    ...
                                                    {/* โปรเจ็ค */}
                                                    <div style={{ marginBottom: 12 }}>
                                                        <Typography.Text strong ellipsis={{ tooltip: entry.project_name }}>
                                                            {entry.project_name}
                                                        </Typography.Text>
                                                    </div>
                                                    ...
                                                    {entry.feature_name && (
                                                        <div style={{ marginBottom: 12 }}>
                                                            <Typography.Text type="secondary" ellipsis={{ tooltip: entry.feature_name }}>
                                                                {entry.feature_name}
                                                            </Typography.Text>
                                                        </div>
                                                    )}
                                                    ...
                                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                                        ...
                                                        <Tag color={statusColorMap[entry.status]}>{statusLabel}</Tag>
                                                    </div>

                                                    {/* คำอธิบาย */}
                                                    {entry.description && (
                                                        <div style={{ marginTop: 12 }}>
                                                            <Typography.Text type="secondary" ellipsis={{ tooltip: entry.description }}>
                                                                {entry.description}
                                                            </Typography.Text>
                                                        </div>
                                                    )}
