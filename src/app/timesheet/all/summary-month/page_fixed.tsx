...
export default function Page() {
    ...
    //* การทำงาน: คำนวณสถิติจากข้อมูลที่กรองแล้วเท่านั้น เพื่อให้ตัวเลขสะท้อนข้อมูลในตาราง
    const summaryCards = useMemo(() => {
        // เปลี่ยนจาก records เป็น filteredRecords เพื่อให้สถิติสะท้อนข้อมูลที่กรองแล้ว
        ...
        const avgCompletion = totalMembers
            ? Number(
                (
                    filteredRecords.reduce((sum, current) => sum + current.completion_rate, 0) /
                    totalMembers
                ).toFixed(2)
            )
            : 0;
    ...
    }, [filteredRecords, metadata]); // เปลี่ยน dependency จาก records เป็น filteredRecords
                    ...
                    {metadata && (
                        <Typography.Text type="secondary">
                            ...
                            {filteredRecords.length !== records.length && (
                                <> | แสดงผล {filteredRecords.length} จาก {records.length} คน</>
                            )}
                        </Typography.Text>
                    )}
