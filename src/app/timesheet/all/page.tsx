...
export default function TimesheetAllPage(): JSX.Element {
  ...
  const statsData = useMemo(() => {
    ...
    const activeProjects = new Set(
      entries.map((entry) => entry.project_name).filter(Boolean)
    ).size;
  ...
  const LOAD_SUB_PROJECTS = useCallback(
  ...
  //** ส่งออกไฟล์ Template */
  const HANDLE_EXPORT_TEMPLATE = useCallback(
  ...
  const HANDLE_EXPORT_TEMPLATE2 = useCallback(
  ...
  const HANDLE_EXPORT_TEMPLATE3 = useCallback(
  ...
  //** ส่งออกไฟล์ Template 4 (Audit Report) */
  const HANDLE_EXPORT_TEMPLATE4 = useCallback(
    async ({ from, to }: { from: string; to: string }) => {
      ...
      try {
        ...
        // Call the new template_4 export endpoint for audit report
  ...
  const handleOpenModal = useCallback(
    (modalType: keyof typeof modalStates, mode?: TimesheetMode) => {
  ...
  return (
    <PermissionLayout role={["ADMIN"]}>
      <DashboardLayout>
        <HeaderBar
          title="Timesheet • All"
          subTitle="จัดการบันทึกเวลาทั้งหมด"
