...
import { Avatar, Button, Descriptions, Modal, Space, Tag, theme, Typography } from "antd";
...
export const DetailModal = () => {
  ...
  return (
    <Modal
      ...
      width={700}
      footer={[
        <Button key="close" onClick={closeDetailModal}>ปิดหน้าต่าง</Button>,
        <Button key="edit" type="primary" icon={<EditOutlined />} onClick={() => router.push(`/admin/user-profile/${selectedUser?.id}`)}>
          แก้ไขข้อมูล
        </Button>,
      ]}
      centered
    >
      {selectedUser && (
        <div className="py-4">
          ...
          <Descriptions title="ข้อมูลส่วนตัวและบัญชี" bordered column={2} className="mb-6">
            ...
            <Descriptions.Item label="อีเมลสำรอง">{(selectedUser as unknown as Record<string, string>).backlog_email || "-"}</Descriptions.Item>
          </Descriptions>

          <Descriptions title="ข้อมูลการทำงาน" bordered column={2} className="mb-6">
