...
// {* แปลงรหัสสถานะ -> ป้ายภาษาไทย + สีแสดงผล *}
const getStatusInfo = (status: string): { label: string; color: string } => {
    switch (status) {
        case "0":
        case "7":
            return {label: "ตรงเวลา", color: "bg-green-600"};
        case "1":
            return {label: "สาย", color: "bg-red-600"};
        case "3":
            return {label: "ขาด", color: "bg-gray-600"};
        case "4":
        case "10":
            return {label: "ลากิจ", color: "bg-yellow-600"};
        case "5":
        case "11":
            return {label: "ลาป่วย", color: "bg-blue-600"};
        case "6":
        case "12":
            return {label: "กิจกรรม", color: "bg-purple-600"};
        case "21":
        case "22":
        case "23":
        case "24":
        case "25":
        case "26":
            return {label: "ลาอื่นๆ", color: "bg-pink-600"};
        case "99":
            return {label: "ไม่เช็กชื่อ", color: "bg-orange-500"};
        case "-":
            return {label: "ไม่ทราบ", color: "bg-gray-400"};
        default:
            return {label: "ไม่ทราบ", color: "bg-gray-400"};
    }
};
...
export default function Page() {
    ...
    const STATISTIC_STATE = useAppSelector(
        (state) => state.callPostStatistic
    );
                ...
                {({index}: { index: number }) => {
                    ...
                    return (
                        <>
                            ...
                            <td className="p-4 font-medium text-sm text-gray-900 dark:text-gray-200">
                                {/* {* ผู้ที่แสกน *} */}
                                {
                                    userList.find((user: any) =>
                                        String(user.value) === String(form.user_id)
                            ...
                            <td className="p-4 font-medium text-sm text-gray-900 dark:text-gray-200">
                  <span
                      className={`inline-block px-3 py-1 text-sm font-semibold text-white ${inInfo.color} rounded-full`}>
                    {inInfo.label}
                  </span>
                            </td>
                            ...
                            <td className="p-4 font-medium text-sm text-gray-900 dark:text-gray-200">
                  <span
                      className={`inline-block px-3 py-1 text-sm font-semibold text-white ${outInfo.color} rounded-full`}>
                    {outInfo.label}
                  </span>
                            </td>
