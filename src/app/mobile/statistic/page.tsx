...
import {Button, Card, Col, DatePicker, Form, Row, Select, Spin, Table, Tag, Typography} from "antd";
...
export default function Page() {
    ...
    //** user options (memoized from USER_LIST_STATE) **
    const userOptions = useMemo(() => {
        console.info("🔄 Recomputing userOptions from USER_LIST_STATE:", USER_LIST_STATE);
        return (
            USER_LIST_STATE?.response?.data?.data?.map(
                (item: type.ResponseUserList["draftValues"]) => ({
                    label: `${item?.Name ?? ""} ${item?.LastName ?? ""} (ID: ${item?.UserID})`,
    ...
    useEffect(() => {
        ...
        if (lastFetchedSchoolId.current === idStr) return; // ✅ ป้องกันเรียกซ้ำ
    ...
    const handleSubmitForm = async (values: any) => {
        ...
        try {
            ...
            if (response?.data?.length > 0) {
                toast.success(`ค้นหาสำเร็จ (รหัสนักเรียน/บุคลากร: ${values.user_id})`, {id: tId});
            } else {
    ...
    const columns = useMemo(
        () => getTableColumns(userOptions, String(selectedUserId ?? ""), STATISTIC_STATE?.response?.curl),
                                ...
                                <Col xs={24} sm={12}>
                                    ...
                                    >
                                        <Select
                                            ...
                                            options={[{label: "เลือกรายการ", value: ""}, ...userOptions]}
