...
function baseBeautifulLog(
  ...
  try {
    ...
    console.error(
      `${logColors.error}[ERROR] Failed to stringify log:${resetColor}`,
      error,
    );
    logFn(data);
  }
...
baseBeautifulLog.log = (data: unknown, space = 2) =>
  baseBeautifulLog(data, "log", space);
baseBeautifulLog.info = (data: unknown, space = 2) =>
  baseBeautifulLog(data, "info", space);
baseBeautifulLog.warn = (data: unknown, space = 2) =>
  baseBeautifulLog(data, "warn", space);
baseBeautifulLog.error = (data: unknown, space = 2) =>
  baseBeautifulLog(data, "error", space);
