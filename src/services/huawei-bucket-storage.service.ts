...
export const HUAWEI_STORAGE = {
  OBS_BUCKET_URL: process.env.OBS_BUCKET_URL ?? "error",
  OBS_ENDPOINT: process.env.OBS_ENDPOINT ?? "error",
  OBS_DOMAIN: process.env.OBS_DOMAIN ?? "error",
  OBS_ACCOUNT_ID: process.env.OBS_ACCOUNT_ID ?? "error", // เพิ่มไว้กรณีต้องใช้ ID ในการระบุ Path
} as const;
